...
class TickLoop
{
	...
	private static function init() {
		...
		declare(ticks=1);
		self::$GO = true;
		register_tick_function(array('verysimple\EventLoop\TickLoop','onTick'));
	...
	static function start() {
		...
		while(self::$GO) usleep(self::$SLEEP_TIME);

		// tick function has to be unregistered or else orphan events will attempt to fire
		unregister_tick_function(array('verysimple\EventLoop\TickLoop','onTick'));
	...
	public function __construct() {
		throw new Exception('TickLoop is a static class and cannot be instantiated');
	}
